...
// 대분류 12개 · 소분류 40개.
//
// 처음에는 세세하게 나눴는데(대분류 24개 · 조합 84개) 입력할 때마다
// 고르는 게 일이 되어 쓰지 않게 됐다. 실제 사용 데이터를 보고 합쳤다.
//   카페 → 식비 안으로 · 쇼핑 → 생활 안으로 · 야식 → 식비(술/간식)
//   소고기·돼지고기·닭고기 → 육고기 · 소주·맥주·와인 → 술
//   계란·두부·조미료·과일·라면 같은 잔가지 → 장보기
export const CATEGORY_MAIN_OPTIONS = [
  '수입', '고정비', '식비', '생활', '교통', '의료',
  '취미', '육아', '여행', '출장', '경조사·모임', '기타'
] as const;
...
export interface DailyTrack {
  ...
  condition_text?: string;       // AI 자동 체크 조건 (자연어 — 예: "12시 전 취침")
