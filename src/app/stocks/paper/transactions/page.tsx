...
export default function PaperTransactionsPage() {
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="sticky top-0 z-10 bg-white border-b border-gray-100">
        <div className="max-w-lg mx-auto px-4 py-3 flex items-center gap-3">
          <Link href="/stocks/paper/portfolio" className="p-2 -ml-2 rounded-xl hover:bg-gray-100">
            <ChevronLeft size={22} className="text-gray-700" />
          </Link>
          <FlaskConical size={16} className="text-violet-500" />
          <h1 className="text-lg font-bold text-gray-900 flex-1">모의 거래내역</h1>
      ...
      <div className="max-w-lg mx-auto px-5 pt-4 space-y-4">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl p-10 shadow-sm border border-gray-100 text-center">
            <p className="text-sm text-gray-500">모의 거래 내역이 없습니다.</p>
            <button
              onClick={() => setSheet({ mode: 'create' })}
              className="mt-4 px-5 py-2.5 rounded-xl bg-violet-600 text-white text-sm font-semibold"
            >
              첫 모의 거래 추가
            </button>
          </div>
