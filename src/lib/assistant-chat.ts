...
E) **주식 입력 — 텍스트 vs 캡쳐 구분:**
   - **텍스트 입력 (리딩방/추천 메시지)** → save_stock_recommendation (메모로만 저장)
   - **증권사 캡쳐 이미지** → 서버가 미리 stock_brokerage 로 분류해서 별도 흐름으로 처리됨 (이 도구 호출 X).
     이미지가 분류 통과 못해 여기까지 왔으면 텍스트 흐름으로 처리.
   원칙: "내가 X 매수했다" 같은 본인 거래 텍스트도 일단 메모로 저장 (캡쳐가 아니면 정확한 거래 등록 어려우니).

F) **save_stock_recommendation** — 주식 추천/리딩방/매매 메시지 자동 저장:
