...
anon public key      → 긴 문자열 (Vercel 에 NEXT_PUBLIC_SUPABASE_ANON_KEY 로 입력)
```

> ℹ️ Settings → API Keys 에 가면 service_role / Secret Keys 도 보이는데 **무시해도 됩니다** — 본 앱은 anon key 만으로 작동합니다.
...
브라우저(Chrome / Edge / Safari) 아무거나 켜놓은 상태에서:

#### 1단계 — "개발자 도구" 라는 창 열기
- [ ] 키보드 맨 윗줄에 있는 **F12** 키 한 번 누르기
   - 위치: 키보드 가장 윗줄, 오른쪽에 가까운 쪽 (Backspace 근처)
   - F12 가 안 먹히면 → **Fn 키를 누른 채로 F12** 같이 누르기 (노트북에서 흔함)
- [ ] 화면 옆이나 아래에 새 패널이 생김 (코드 막 적혀있는 까만/회색 화면)

> 💡 이 패널은 **개발자 도구** 라고 해요. 평소엔 안 쓰지만 이번 한 번만 사용.

#### 2단계 — Console 탭 클릭
- [ ] 그 새 패널 위쪽에 여러 글자 메뉴 있음 (Elements / **Console** / Network / ... )
- [ ] **Console** 글자 클릭 → 빈 칸이 보이면 OK

#### 3단계 — 코드 한 줄 붙여넣기
- [ ] 아래 코드 박스의 **복사** 아이콘 클릭 (또는 마우스로 전체 드래그 후 Ctrl+C)

```js
btoa(crypto.getRandomValues(new Uint8Array(48)).reduce((a,b)=>a+String.fromCharCode(b),''))
```

- [ ] Console 의 빈 칸에 붙여넣기 (Ctrl+V) → 그리고 **Enter** 키 누름
- [ ] 그 줄 아래에 길고 이상한 문자열이 짠 하고 나타남
   - 예: `"a3F9c8e2B7d1...64글자"` 같은 거

> ⚠️ 처음에 ![Paste warning](https://i.imgur.com/F12warning.png) 같은 경고가 뜨면 그 칸에 **`allow pasting`** 다섯 글자 타이핑 후 Enter → 그러고 나서 다시 위 코드 붙여넣기.

#### 4단계 — 그 문자열 메모해두기
- [ ] 출력된 64글자 문자열을 **마우스로 드래그해서 복사** (양 옆 따옴표는 빼고 안의 글자만)
- [ ] 메모장에 붙여넣고 라벨: `APP_ENCRYPTION_KEY`

이 키는 나중에 5단계(Vercel 환경변수)에서 사용합니다.

> ⚠️ **중요**: 이 키 잃어버리면 저장된 토큰들 복호화 불가. 메모장 외에 클라우드(구글 드라이브 등)에도 백업해두기.
> ⚠️ 절대 다른 사람과 공유하지 말 것.
...
| `APP_PASSWORD` | 본인이 만들 비밀번호 | 앱 로그인할 때 입력할 비번. 영문+숫자 8자 이상 |
| `AUTH_SECRET` | 또 다른 랜덤 문자열 | 4번 방법(F12 콘솔) 으로 한 번 더 만들어서 붙여넣기 |
| `NEXT_PUBLIC_DISABLE_STOCKS` | `true` | 주식 메뉴 숨김 (글자 그대로 `true`) |
| `NEXT_PUBLIC_SOLO_MODE` | `true` | 1인 모드 (가족 UI 숨김. 글자 그대로 `true`) |

**선택 항목 (안 넣어도 앱은 정상 작동)**

| Name (변수명) | Value (값) | 비고 |
|---|---|---|
| `USD_KRW` | `1380` | AI 비용 원화 환산 환율. 안 넣으면 1380 기본 |
| `CRON_SECRET` | 또 다른 랜덤 문자열 | (브리핑 푸시 자동화 할 때만) cron-job.org 인증용 |

> 💡 푸시 알림 / 노션 / R2 / Google 캘린더 환경변수는 가이드 뒤쪽 각 섹션에서 별도 안내합니다 — 여기에선 안 넣어도 됩니다.
...
푸시 알림용 키 2개를 만들어야 해요. 웹사이트 한번 방문하면 자동으로 만들어집니다.

- [ ] 다음 링크 클릭 (새 탭에서 열기 권장):
      👉 **https://www.attheminute.com/vapid-key-generator**
- [ ] 페이지 들어가자마자 두 개의 긴 문자열이 자동 표시됨:
      - **Public Key** (Application Server Public Key) — 첫 번째
      - **Private Key** (Application Server Private Key) — 두 번째
- [ ] 두 값 각각 옆의 복사 버튼 클릭 → 메모장에 라벨 붙여서 저장
      ```
      VAPID_PUBLIC_KEY = BLn4...긴문자열
      VAPID_PRIVATE_KEY = zXk8...긴문자열
      ```

> 💡 같은 사이트를 새로고침하면 매번 새 키가 생성됩니다 — 처음 한 번 만들어진 두 키만 저장해두면 OK. (다시 만들면 푸시 동작 안 함)
...
cron-job.org 가입 → **CREATE CRONJOB** → 아래 작업 등록:

| Title | URL | Schedule |
|---|---|---|
| 아침 브리핑 | `https://본인앱URL/api/briefing?mode=morning&push=1` | 매일 07:00 |
| 저녁 브리핑 | `https://본인앱URL/api/briefing?mode=evening&push=1` | 매일 22:00 |
| 할일 알림 | `https://본인앱URL/api/tasks/reminders/dispatch` | 5분마다 (`*/5 * * * *`) |
| 데일리 트랙 알림 | `https://본인앱URL/api/daily-tracks/reminders/dispatch` | 5분마다 (`*/5 * * * *`) |

**선택 사항 (관련 기능 쓸 때만 추가):**
| 텔레그램 알림 | `https://본인앱URL/api/todo/telegram/dispatch` | 5분마다 |
| 주식 자산 추세 스냅샷 | `https://본인앱URL/api/stocks/asset-history/snapshot` | 매일 16:00 |
| 주간 리포트 | `https://본인앱URL/api/reports/weekly` | 매주 일 21:00 |

> 💡 **Timezone**: cron-job.org 의 timezone 을 **Asia/Seoul** 로 설정하면 위 시각이 한국시간 기준으로 동작.
> 💡 **CRON_SECRET 불필요**: 미들웨어가 위 dispatch 경로들 인증 우회하므로 secret 안 붙여도 됨.
...
### Naver CLOVA OCR — 한국 영수증 정확도 향상 (선택, 거의 무료)

> 💡 **건너뛰어도 됨.** 안 하면 gpt-4o vision 으로 자동 fallback. 한국 마트 영수증 정확도만 살짝 높이고 싶을 때 추가.

**비용:** 일반 (General) 도메인 + General 플랜 = 호출당 1~2원, 월 기본요금 0원. 영수증 100장/월 ≈ 100~200원.

**셋업 (10분):**
1. https://www.ncloud.com 가입 (네이버 계정 연동 가능)
2. 콘솔 → AI Service → **CLOVA OCR** → 이용 신청
3. **Domain 생성**:
   - 이름: `moneykeeper-receipt`
   - 도메인 코드: `mkreceipt`
   - 지원 언어: 한국어
   - 서비스 타입: **일반** (Template 은 월 기본요금 발생, 피하기)
4. NCP 콘솔 → **API Gateway** 이용 신청 (별도 무료 활성화 필요)
5. CLOVA OCR 도메인 → **API Gateway 연동** 클릭 → 자동 등록
6. 등록 후 표시되는 두 값 복사:
   - **APIGW Invoke URL** (`https://xxxxx.apigw.ntruss.com/custom/v1/...`)
   - **Secret Key**
7. Vercel Env Variables 추가:
   - `CLOVA_OCR_INVOKE_URL` = (Invoke URL)
   - `CLOVA_OCR_SECRET_KEY` = (Secret Key)
8. Vercel Redeploy

**확인:** 앱에서 영수증 업로드 → 인식 정확도 향상 (특히 마트 영수증). 실패 시 자동으로 gpt-4o 로 fallback.

---

### Cloudflare R2 (10GB 무료 — 파일 첨부할 때만)

> **이거 꼭 필요한가?** → 아니요. **건너뛰어도 됩니다.** 가계부 · 할일 · 활동 추적 · AI 어시스턴트 · 브리핑 · 영수증 OCR · 푸시 알림 — **전부 R2 없이 정상 작동**합니다.

**언제 필요한지 (이 중 하나라도 해당하면 설정 추천):**
- 📚 **아카이브 컬렉션에 이미지/PDF 첨부**할 일이 자주 있을 때 (예: 책 표지, 영수증 사본, 자격증 사본 등)
- 📥 **노션에서 이미지 포함된 페이지 임포트**할 때 (이미지가 있어야 컬렉션에 그대로 들어감)

**언제 안 해도 되는지:**
- 텍스트/숫자/체크박스/별점 같은 **일반 속성만 쓸 때**
- 영수증은 OpenAI 가 즉시 읽고 거래로 등록하지 **저장은 안 합니다** (R2 불필요)
- 가계부 위주로 쓰고 아카이브는 안 쓸 때

**나중에 추가해도 됨:**
처음 설정 안 하고 쓰다가 "파일 첨부 미설정" 에러 보이면 그때 추가해도 충분합니다. 환경변수만 더 넣고 Vercel 재배포하면 끝.

**설정 방법** (필요할 때):
fork 한 저장소의 `docs/R2-SETUP.md` 파일 참고 (GitHub 에서 그 경로로 들어가면 보임). 약 5분 소요.
